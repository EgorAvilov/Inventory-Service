...
public class IngredientCommandServiceImplTest {
    ...
    public void updateAmountNoItemExceptionNoSuchIngredient() {
        ...
        when(ingredientRepository.findByNameAndRestaurantId(ingredient.getName(), restaurant.getId())).thenThrow(new NoItemException("No such ingredient"));
    ...
    public void updateAmount() {
        ...
        //when
        when(ingredientConverter.dtoToEntity(ingredientUpdateAmountDto)).thenReturn(ingredient);
        when(ingredientRepository.findByNameAndRestaurantId(ingredient.getName(), restaurant.getId())).thenReturn(Optional.of(persistIngredient));
    ...
    @Test(expected = NoItemException.class)
    public void updateNoItemExceptionNoSuchIngredient() {
        //given
        IngredientUpdateDto ingredientUpdateDto = IngredientUpdateDto
                .builder()
                .name("name")
        ...
        ingredientService.update(ingredientUpdateDto);
        //then
    ...
    public void update() {
        ...
        //when
        when(ingredientConverter.dtoToEntity(ingredientUpdateDto)).thenReturn(ingredientUpdate);
        when(ingredientRepository.findByNameAndRestaurantId(ingredientUpdate.getName(),ingredientUpdate.getRestaurant().getId())).thenReturn(Optional.of(persistIngredient));
        when(ingredientRepository.save(persistIngredient)).thenReturn(persistIngredient);
        ingredientService.update(ingredientUpdateDto);
        //then
        verify(ingredientConverter).entityToDto(persistIngredient);
        assertThat(persistIngredient).extracting(Ingredient::getAmount)
                .isEqualTo(ingredientUpdateDto.getAmount()
                        .add(amount));
    }
